...
import { Loader2, Award, Calendar, Trash2, ShieldAlert } from "lucide-react";
...
export function StudentIjazatTab({ studentId, isAdmin = false }: StudentIjazatTabProps) {
  ...
  const fetchIjazat = async () => {
    try {
      const res = await fetch(`/api/ijazat?student_id=${studentId}`);
      if (!res.ok) {
        throw new Error("حدث خطأ أثناء تحميل سجل الإجازات");
      }
  ...
  const handleRevoke = async (id: string) => {
    ...
    setRevokingId(id);
    try {
      const res = await fetch(`/api/ijazat/${id}`, {
        method: "DELETE",
      });

      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.error || "فشل إلغاء الإجازة");
      }
